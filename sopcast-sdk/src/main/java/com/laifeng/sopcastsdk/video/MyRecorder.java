...
public class MyRecorder {
    ...
    public boolean firstTimeSetup() {
        ...
        try {
            mInputSurface = new InputSurface(mMediaCodec.createInputSurface());//数据传入
            mMediaCodec.start();//开始记录视频
        } catch (Exception e) {
            releaseEncoder();
        ...
        while (isStarted) {
            ...
            if (mMediaCodec != null) {
                ...
                if (outBufferIndex >= 0) {
                    ...
                    if (mListener != null) {
                        mListener.onVideoEncode(bb, mBufferInfo);//视频流的回调到StreamController
                    }
